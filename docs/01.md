...
[Python](https://www.python.org/) je jeden z mnohých *programovacích jazykov*. Tak ako ľudia medzi sebou sa rozprávame spoločným jazykom, potrebujeme sa naučiť nový jazyk, ktorému rozumie počítač či robot. Tak mu budeme vedieť povedať, kedy sa pohnúť, ako rýchlo, čo zobraziť na displeji a podobne.
...
![Python IDLE](img/python_shell_win.png)
...
okne, ktoré sa nám otvorilo, môžme zadávať jednotlivé príkazy. Na poslednom
riadku vydíme tzv. *prompt*: `>>>`. Označuje, že program je pripravený počúvať
...
keď použijeme na delenie `//` namiesto `/`. Pre túto operáciu sa používa znak
`%`.
...
Príkaz `x = 4 + 8` znamená priradenie hodnoty na pravej strane do *premennej*
na ľavej strane. Najprv Python vypočíta `4 + 8`. Potom na miesto v pamäti, kde
má uložený výsledok, si nalepí poznámku "x". Je to názov novej premennej, aby
si pamätal kam sa má pozrieť, keď ju uvidí v našich príkazoch nabudúce.
Napr. `2 * x` -> `2 * 12` -> `24`.
...
ale tento výsledok sa stratí, nemá žiadny príkaz, aby ho vypísal. Príkazový
riadok nám vždy aj ukáže výsledok toho, čo mu zadáme.
...
(predposledné dva riadky začínajú štyrmi medzerami):
...
`print( 'jablko' )`), a keď vytvoríme nejaký úžasný program, budeme sa môcť s
ním podeliť s celým svetom!
...
interpretovať daný riadok ako príkaz a skončil s chybovou hláškou.
...
pyramídu. Tu su vzory pre veľkosti 1, 2 a 4:
